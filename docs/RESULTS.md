...
| Версия | Macro F1 | Итог |
|---|---:|---|
| **v45** | **0.8644397759** | **8-е место, прохождение в этап проверки репозиториев** |

Private test составлял основную закрытую часть оценки и не использовался для
выбора параметров. Score опубликован после фиксации финального submission.
...
| v19 + 5 regex v44, без standardized head | F1 0.768421 | +0.094200 | отдельно не отправлялся |
| **standardized + regex v45** | **F1 0.775726** | **+0.101505** | **public 0.814222; принят** |

v27 показал, что outer proxy может быть оптимистичным, если lookup построен по
полному labeled dataset. Поэтому из v45 исключены exact-title и hard-nearest
механизмы. Regex v45 тоже отбирались по всему released train, поэтому их outer
gain является model-selection proxy, а не независимой оценкой. Public-прирост
`+0.005329` заметно скромнее локального proxy и является внешним наблюдением для
двух полных систем, но public использовался при выборе версии и не является
unbiased validation.

### Ablation финальных компонентов

| Версия | ML/kNN | Standardized head | 5 regex | Flammable outer F1 |
|---|:---:|:---:|:---:|---:|
| v19 | ✓ | — | — | `0.674221` |
| v34 | ✓ | ✓ | — | `0.681818` |
| v44 | ✓ | — | ✓ | `0.768421` |
...
Следовательно, на этом model-selection proxy regex дали основную часть прироста
после v19, а standardized head — меньший независимый сигнал. Это не превращает
финал в regex-only classifier: v19 без этих пяти правил уже получил public
`0.808892`. Но называть правила несущественными также было бы неверно.

Public сравнивает только v19 и v45 целиком. Private-ablation невозможна, потому
что на private был рассчитан только финальный v45, а истинные метки и component
F1 организаторы не публикуют.
...
Полный список 34 запусков с runtime, VRAM и причиной принятия/отклонения: [experiments.csv](../experiments.csv).
