...
> Я решал бинарную модерацию товаров в двух категориях по названию, описанию и изображениям. Главная особенность данных — противоположный баланс классов: для БАД 74.5% положительных карточек, а для воспламеняемых только 3.6%. Поэтому я строил отдельные category heads и оптимизировал официальный positive F1. Финальная v45 получила public Macro F1 0.814222 и private Macro F1 0.864440, заняв 8-е место итогового рейтинга.
